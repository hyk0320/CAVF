...
def get_forword_results(opt, model, data, device, only_data=False, vocab=None, **kwargs):
    # category, labels, new_module_labels = map(
    #         lambda x: x.to(device),
    #         [data['category'], data['labels'], data['new_module_labels']]
    #     )
    ...
    if opt['visual_word_generation']:
        ...
        mlm_tgt_tokens = [data['labels_1'].to(device), data['labels'].to(device)]
        full_sequence = data['full_sequence'].to(device)
        verb = True
        gt_many_hot_verb = data['verb_label'].to(device)
    else:
        tokens = data['tokens'].to(device)
        mlm_tgt_tokens = data['labels'].to(device)
        full_sequence = data['full_sequence'].to(device)
        verb = True
        gt_many_hot_verb = data['verb_label'].to(device)
    ...
    if only_data:
        # for evaluation
        results = model.encode(feats=feats, gt_many_hot_verb=None)
        # print('reslut of encode in run:', results)
        # print('reslut of encode[enc_output]:', results['enc_output'].shape)
        # print('reslut of encode[pred_length]:', results['pred_length'].shape)
        # print('reslut of encode[enc_hidden]:', results['enc_hidden'].shape)
    else:
        if opt['is_verb_decoder']:
            print('run79')
            results = model(    # {'pred_length': , 'tgt_word_logprobs': , 'new_module_prediction': ,'layer1': , 'layer2': ,}
                feats=feats,
                verb=verb,
                tgt_tokens=tokens,
                ...
                vocab=vocab,
                device=device,
                gt_many_hot_verb=gt_many_hot_verb,
                **kwargs
            )
            results['verb_label'] = data['verb_label'].to(device)
        else:
            results = model(    # {'pred_length': , 'tgt_word_logprobs': , 'new_module_prediction': ,'layer1': , 'layer2': ,}
                feats=feats,
                tgt_tokens=tokens,
    ...
    if opt['decoding_type'] == 'NARFormer':
        results[Constants.mapping['length'][1]] = prepare_data(data, 'length_target', device)  #   {'pred_length': , 'tgt_word_logprobs': , 'new_module_prediction': ,'layer1': , 'layer2': , 'tgt_length': }
        start_index = 0
    else:
        start_index = 1
    ...
    if opt['group_dependency']:
        results['new_module_labels'] = new_module_labels[:, start_index:]
...
def run_eval(
    ...
    for data in tqdm(loader, ncols=150, leave=False):
        with torch.no_grad():
            ...
            if crit is not None:
                if opt['is_FCNet']:
                    FCNet_loss, _ = crit.get_loss(encoder_outputs)
                else:
            ...
            all_hyp, all_scores = translator.translate_batch(encoder_outputs, category, labels, vocab, teacher_encoder_outputs=teacher_encoder_outputs, device=device)
            if opt['batch_size'] == 1:
        ...
        for k, hyps in enumerate(all_hyp):
            ...
            print("epoch:", global_step, "video:", video_id, sentence)
    ...
    if opt['batch_size'] == 1:
        ...
        print("latency", latency, len(loader))
...
def run_train(opt, model, crit, optimizer, loader, device, logger=None, epoch=-1, return_all_info=False, **kwargs):
    ...
    for data in tqdm(loader, ncols=150, leave=False):
        ...
        if opt['is_FCNet']:
            FCNet_loss, loss = crit.get_loss(results, epoch=epoch)
        elif opt['is_BCNet']:
            # BCNet_loss1, BCNet_loss2, loss = crit.get_loss(results, epoch=epoch)
            BCNet_loss1, loss = crit.get_loss(results, epoch=epoch)
        elif opt['is_NAR_decoder']:
            NAR_loss, loss = crit.get_loss(results, epoch=epoch)
        else:
            NAR_loss, loss = crit.get_loss(results, epoch=epoch)
        # print('new_module_loss:'+str(new_module_loss))
        # print('总loss:'+str(loss))
        loss.backward()
...
def train_network_all(opt, model, device, **kwargs):
    print("run337", opt.get('is_verb_decoder', False))
    ...
    for epoch in tqdm(range(opt['epochs']), ncols=150, leave=False):
        ...
        if (epoch+1) > opt['start_eval_epoch'] and (epoch+1) % opt["save_checkpoint_every"] == 0:
            print("here is {}th start of validation:".format(epoch))
            ...
            print('here is {}th end of validation'.format(epoch))
    ...
    if not opt.get('no_test', False):
        ...
        print('here is start of test:')
        os.system('python translate.py --default --algorithm_print_sent -paradigm {} -latency --method {} --dataset {} --record --scope {} --field {} --val_and_test --use_ct'.format(
            opt['paradigm'], opt['method'], opt['dataset'], opt['scope'] if opt['scope'] else '\"\"', ' '.join(opt['field']))
        )
        print('here is end of test')
