...
class Algorithm_Base(object):
    ...
    def get_coarse_grained_templates(self, model, inputs_for_decoder, tgt_tokens, pad_mask, action_prediction=None, verb=False):
        mask_ind = tgt_tokens.eq(Constants.MASK)
        tgt_tokens[mask_ind] = self.visual_tag
        if self.opt['is_verb_decoder']:
            tgt_tokens, token_probs, attentions = self.generate_non_autoregressive(model, inputs_for_decoder, tgt_tokens, pad_mask, is_ct=True, action_prediction=action_prediction, verb=verb)
        else:
    ...
    def generate_non_autoregressive(self, model, inputs_for_decoder, tgt_tokens, pad_mask, zeros=[], tag_replace=None, return_all_probs=False, is_ct=False, debug=False, iteration=0, device=None, action_prediction=None, verb=False):
        # LSTM_squence_output, decoder_out, _, attentions = model.decoder(tgt_tokens, **inputs_for_decoder, output_attentions=True, iteration=iteration, device=device)
        if self.opt['is_NAR_decoder']:
            decoder_out, _, attentions = model.decoder(tgt_tokens, **inputs_for_decoder, output_attentions=True, iteration=iteration, device=device)
        elif self.opt['is_visual_decoder']:
            decoder_out, _, attentions = model.decoder(tgt_tokens, **inputs_for_decoder, output_attentions=True, iteration=iteration, device=device)
        elif self.opt["is_verb_decoder"]:
            # print("algorithm150：", action_prediction)
            decoder_out, _, attentions = model.decoder(verb, tgt_tokens, **inputs_for_decoder, output_attentions=True, iteration=iteration, device=device, action_prediction=action_prediction)
    ...
    def scoring_by_teacher(self, teacher_model, teacher_inputs_for_decoder, tgt_tokens, pad_mask, is_last=False):
        ...
        # forward
        # decoder_out, *_ = teacher_model.decoder(False, tgt_tokens_with_bos[:, :-1], **teacher_inputs_for_decoder)
...
class MaskPredict(Algorithm_Base):
    def __init__(self, opt, dict_mapping, tgt_vocab):
        ...
        self.Is_verb_decoder = opt.get("is_verb_decoder", False)

    def generate(self, model, teacher_model, inputs_for_decoder, teacher_inputs_for_decoder, tgt_tokens, action_prediction=None, device=None):
        bsz, seq_len = tgt_tokens.size()
        # print('bs {} in algorithms'.format(bsz))
        pad_mask = tgt_tokens.eq(Constants.PAD)
        seq_lens = seq_len - pad_mask.sum(dim=1)

        if self.Is_verb_decoder:
            if self.use_ct:
                tgt_tokens, token_probs, attentions = self.get_coarse_grained_templates(model, inputs_for_decoder, tgt_tokens, pad_mask, action_prediction=action_prediction, verb=True)
            else:
                tgt_tokens, token_probs, attentions = self.generate_non_autoregressive(model, inputs_for_decoder, tgt_tokens, pad_mask, iteration=0, device=device, verb=True)
        else:
            if self.use_ct:
                tgt_tokens, token_probs, attentions = self.get_coarse_grained_templates(model, inputs_for_decoder, tgt_tokens, pad_mask)
            else:
                tgt_tokens, token_probs, attentions = self.generate_non_autoregressive(model, inputs_for_decoder, tgt_tokens, pad_mask, iteration=0, device=device)
        ...
        for counter in range(1, T):
            ...
            # Predict
            if self.Is_verb_decoder:
                new_tgt_tokens, new_token_probs, attentions = self.generate_non_autoregressive(model, inputs_for_decoder, tgt_tokens, pad_mask, iteration=counter, device=device, action_prediction=action_prediction, verb=True)
            else:
                new_tgt_tokens, new_token_probs, attentions = self.generate_non_autoregressive(model, inputs_for_decoder, tgt_tokens, pad_mask, iteration=counter, device=device)
...
class Left2Right(Algorithm_Base):
    ...
    def generate(self, model, teacher_model, inputs_for_decoder, teacher_inputs_for_decoder, tgt_tokens, action_prediction=None, device=None):
        ...
        attentions = None
        if self.use_ct:
            tgt_tokens, token_probs, attentions = self.get_coarse_grained_templates(True, model, inputs_for_decoder, tgt_tokens, pad_mask, action_prediction=action_prediction)
        ...
        for counter in range(0, seq_len, self.q):
            ...
            tgt_tokens[mask_ind] = Constants.MASK
            # Predict
            new_tgt_tokens, new_token_probs, attentions = self.generate_non_autoregressive(True, model, inputs_for_decoder, tgt_tokens, pad_mask, iteration=counter, device=device, action_prediction=action_prediction)
        ...
        for i in range(self.T):
            ...
            tgt_tokens[mask_ind] = Constants.MASK
            new_tgt_tokens, new_token_probs, attentions = self.generate_non_autoregressive(True, model, inputs_for_decoder, tgt_tokens, pad_mask, device=device, action_prediction=action_prediction)
