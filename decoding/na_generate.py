...
def generate(
    ...
    length_beam_size = opt['length_beam_size']  # 6
    ...
    beam = predict_length_beam(gold_target_len, pred_length, length_beam_size, length_bias, opt)   # bs * 6
    ...
    if teacher_encoder_outputs is not None:
        ...
        for key in teacher_inputs_for_decoder.keys():
            ...
            # print('in na_generate, teacher_inputs_for_decoder.keys:', teacher_inputs_for_decoder.keys())
            # print('in na_generate, teacher_inputs_for_decoder:', teacher_inputs_for_decoder)
    else:
        teacher_inputs_for_decoder = None

    # print("NA_generate69:", encoder_outputs["verb"])
    if opt.get("is_verb_decoder", False):
        hypotheses, lprobs, collect_results = algorithm.generate(model, teacher_model, inputs_for_decoder, teacher_inputs_for_decoder, tgt_tokens, action_prediction=encoder_outputs["verb"][1], device=device)
    else:
        hypotheses, lprobs, collect_results = algorithm.generate(model, teacher_model, inputs_for_decoder, teacher_inputs_for_decoder, tgt_tokens, device=device)
...
def predict_length_beam(gold_target_len, predicted_lengths, length_beam_size, length_bias, opt):
    if gold_target_len is not None:
        ...
        # print(type(beam), beam.size(), beam)
