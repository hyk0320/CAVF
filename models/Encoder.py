...
class MultipleStreams(nn.Module):
    ...
    def forward(self, input_feats):
        ...
        # # add global visual features
        # average1 = torch.sum(input_feats[0], dim=1, keepdim=True)/8  # bs * 1 * dv
        # average2 = torch.sum(input_feats[1], dim=1, keepdim=True)/8  # bs * 1 * dv
        # input_feats[0] = torch.cat((input_feats[0], average1), dim=1)
        # input_feats[1] = torch.cat((input_feats[1], average2), dim=1)
        # print(input_feats[0].shape, input_feats[1].shape)
