...
class Translator(object):
    ...
    def translate_batch_ARFormer(self, encoder_outputs, category):
        ...
        def beam_decode_step(
            ...
            def predict_word_BC(dec_seq, inputs_for_decoder, n_active_inst, n_bm):
                original_dec_output, dec_output1, dec_output2, *_ = self.model.decoder(dec_seq, **inputs_for_decoder)
                if isinstance(dec_output2, list):
                    dec_output2 = dec_output2[-1]
                dec_output2 = dec_output2[:, -1, :]  # Pick the last step: (bh * bm) * d_h

                word_prob = self.model.tgt_word_prj(dec_output2)
            ...
            def predict_word_FC(dec_seq, inputs_for_decoder, n_active_inst, n_bm):
                original_dec_output, dec_output1, *_ = self.model.decoder(dec_seq, **inputs_for_decoder)
                if isinstance(dec_output1, list):
                    dec_output1 = dec_output1[-1]
                dec_output1 = dec_output1[:, -1, :]  # Pick the last step: (bh * bm) * d_h

                word_prob = self.model.tgt_word_prj(dec_output1)
                word_prob = F.log_softmax(word_prob, dim=1)
                #print(word_prob[0, :10])
                word_prob = word_prob.view(n_active_inst, n_bm, -1)
        ...
        with torch.no_grad():
            inputs_for_decoder = self.model.prepare_inputs_for_decoder(encoder_outputs, category)  # dictionary
            #-- Repeat data for beam search
            n_bm = self.opt["beam_size"]
            n_inst = inputs_for_decoder['enc_output'].size(0)  # batchsize
    ...
    def translate_batch_NARFormer(self, encoder_outputs, teacher_encoder_outputs, category, tgt_tokens, tgt_vocab, device=None, **kwargs):
    ...
    def translate_batch(self, encoder_outputs, category, tgt_tokens, tgt_vocab, teacher_encoder_outputs=None, device=None, **kwargs):
        if self.opt['decoding_type'] == 'NARFormer':
            return self.translate_batch_NARFormer(encoder_outputs, teacher_encoder_outputs, category, tgt_tokens, tgt_vocab, device, **kwargs)
