...
class BertDecoder(nn.Module):
    ...
    def forward(self, tgt_seq, enc_output=None, mlm_tgt_tokens=None, full_sequence=None, tgt_word_prj=None, category=None, signals=None, tags=None, iteration=0, device=None, **kwargs):
        """
            args:
                tgt_seq: inputs of decoder [6, 12, [mask], 67, ..., [pad]]
                mlm_tgt_tokens: labels of decoder [[pad], [pad], 3, [pad], ..., [pad]]
...
class BertDecoderDisentangled(nn.Module):
    ...
    def forward_(self, tgt_seq, enc_output, mlm_tgt_tokens=None, full_sequence=None, tgt_word_prj=None, category=None, **kwargs):
        if mlm_tgt_tokens is not None:
            seq_probs, layer1, layer2, mlm_tgt_tokens_embeddings, embs, *_ = self.bert(tgt_seq, enc_output, mlm_tgt_tokens, full_sequence, tgt_word_prj, category, **kwargs)
            ...
            seq_probs, embs, *_ = self.bert(tgt_seq, enc_output, mlm_tgt_tokens, full_sequence, tgt_word_prj, category, **kwargs)
            seq_probs = seq_probs[0]
            if len(_):
                return seq_probs, embs, _
            return seq_probs, embs

    def forward(self, tgt_seq, enc_output, mlm_tgt_tokens=None, full_sequence=None, tgt_word_prj=None, category=None, **kwargs):
        if isinstance(enc_output, list):
            assert len(enc_output) == 1
            enc_output = enc_output[0]
        ...
        if isinstance(tgt_seq, list):
            ...
            if mlm_tgt_tokens is not None:
                ...
                seq_probs1, layer3, layer4, mlm_tgt_tokens_embeddings,  *_ = self.forward_(tgt_seq[0], enc_output, mlm_tgt_tokens[0], full_sequence, tgt_word_prj, category, **kwargs)
                # print("noun , verb:", tgt_seq[0][0], tgt_seq[0].shape, mlm_tgt_tokens[0], mlm_tgt_tokens[0].shape)
                seq_probs2, embs = self.forward_(tgt_seq[1], enc_output, None, full_sequence, tgt_word_prj, category, **kwargs)
            ...
            if mlm_tgt_tokens is None:
                return self.forward_(tgt_seq, enc_output, mlm_tgt_tokens, full_sequence, tgt_word_prj, category, **kwargs)
            else:
