...
class LSTM_SQUENCE(nn.Module):
    ...
    def forward(self, pos_embeddings, tgt_seq, enc_output, device, hidden_states=None, iteration=0):
        ...
        if self.lenOfTgt < 30:
            ...
            label_EnAndPos1 = torch.index_select(self.hidden_states - self.pos_embeddings, 1, torch.linspace(1, self.lenOfTgt-1, self.lenOfTgt-1).long().to(device))
            ...
            label_EnAndPos2 = torch.index_select(self.hidden_states - self.pos_embeddings, 1, torch.linspace(2, self.lenOfTgt-1, self.lenOfTgt-2).long().to(device))#bs * 28 *512
