...
class BertDecoder(nn.Module):
    ...
    def forward(self, tgt_seq, enc_output=None, category=None, signals=None, tags=None, iteration=0, device=None, **kwargs):
        ...
        if self.BCNet:
            ...
            #     res_BCNet.append(layer_outputs_sim[0])

            # refine existing generated sequence
            for k, layer_module in enumerate(self.layer_visual):
                if not k:
                    input_ = layer_outputs[0]
                else:
                    input_ = layer_outputs_visual[0]

                layer_outputs_visual = layer_module(
                    input_,
                    non_pad_mask=non_pad_mask,
                    attention_mask=slf_attn_mask_sim,  # 第一层mask self attetion 的mask
                    enc_output=enc_output,
                ...
                res_BCNet.append(layer_outputs_visual[0])

            # res_BCNet = [res_BCNet[-1]]
            # print(layer_outputs_sim)

        if additional_feats == None and position_embeddings == None:
            if self.BCNet:
                # outputs = (res, res_BCNet[0], res_BCNet[1], embs,)
                outputs = (res, res_BCNet[0], embs,)
            else:
                outputs = (res, embs)
        else:
            if self.BCNet:
