...
# 简单的Attention操作，必须给出mask
class BertSelfAttention(nn.Module):
    def __init__(self, config, attend_to_enc_output=False):
        super(BertSelfAttention, self).__init__()
        # print(config.is_NAR_decoder)
        num_heads = getattr(config, 'num_attention_heads_c', config.num_attention_heads)
        print(num_heads)
        if config.dim_hidden % num_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.dim_hidden, num_heads))

        self.num_attention_heads = num_heads
        self.attention_head_size = int(config.dim_hidden / num_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size# config.num_attention_heads
        ...
        self.use_sigmoid_to_get_attprob = getattr(config, 'use_sigmoid_to_get_attprob', False)# False
    ...
    def forward(self, q, k, v, attention_mask, head_mask=None, output_attentions=False):
        ...
        attention_scores = torch.bmm(q, k.transpose(1, 2))#矩阵相乘 Q * K（T）
...
# 线性映射一下Attention的结果512-》512，并且做dropout防止过拟合
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = BertLayerNorm(config.dim_hidden, eps=config.layer_norm_eps) if config.with_layernorm else None#    没啥用 None
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
# 连接BertSelfAttention和BertSelfOutput
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.dim_hidden, config.intermediate_size)# 512-》2048
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.intermediate_size, config.dim_hidden)#    2048-》512
        self.LayerNorm = BertLayerNorm(config.dim_hidden, eps=config.layer_norm_eps) if config.with_layernorm else None#    None
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class cross_attention_layer_1(nn.Module):
    def __init__(self, config, is_decoder_layer=False):
        super(cross_attention_layer_1, self).__init__()
        is_parallel_mlm = getattr(config, 'parallel_mlm', False)#   False
    ...
    def forward(self, hidden_states, non_pad_mask=None, attention_mask=None,
                ...
                output_attentions=False, head_mask=None, position_embeddings=None, sim_or_visual=1, **kwargs):
        all_attentions = ()

        if sim_or_visual:   # 0 for visual, 1 for sim
            ...
            attention_output = attention_outputs[0]  # attention result
            ...
            if self.attend_to_enc_output is not None:
                ...
                attention_outputs = self.attend_to_enc_output(
                    hidden_states, enc_output, enc_output,
        ...
        # layer_output = torch.softmax(layer_output, dim=-1)

        # embs = layer_output.sum(1) / non_pad_mask.sum(1)
        # outputs = (layer_output, embs,) + (all_attentions, )  # add attentions if we output them
        embs = attention_output.sum(1) / non_pad_mask.sum(1)
        outputs = (attention_output, embs,) + (all_attentions, )
        return outputs
