...
class Predictor_length(nn.Module):
    ...
    # def forward(self, enc_output, action_prediction, **kwargs):
    def forward(self, enc_output, **kwargs):
        ...
        # assert len(action_prediction.shape) == 2  # bs * dim

        # action_prediction = action_prediction.unsqueeze(1).repeat(1, enc_output.size(1), 1)

        print("P33:", enc_output.size())
        # out = self.net(enc_output.mean(1)+action_prediction)
