...
class BertDecoder(nn.Module):
    def __init__(self, config, embedding=None):
        ...
        # 这里需要加一个LSTM_squence的实例，引入我们的模块
        # if config.version == 'v2':
        self.partially = LSTM_squence.LSTM_SQUENCE(config)
        # else:
        #     self.partially = LSTM_squence_v2.LSTM_SQUENCE(config)

        # print('self.embedding:', self.embedding)
        # print('self.embedding.word_embeddings:', self.embedding.word_embeddings)
        # print('self.embedding.position_embeddings', self.embedding.position_embeddings)
        # print('self.embedding.category_embeddings', self.embedding.category_embeddings)
    ...
    def forward(self, tgt_seq, enc_output=None, category=None, signals=None, tags=None, iteration=0, device=None, **kwargs):
        ...
        for i, layer_module in enumerate(self.layer):
            if not i:
                ...
                if additional_feats == None or position_embeddings == None:
                    input_ = hidden_states
                elif position_embeddings.size(1) < 30:#for test
                    # print('position_embeddings.shape:', position_embeddings.shape)
                    # print('additional_feats.shape:', additional_feats.shape)
                    input_, prediction = self.partially(position_embeddings, tgt_seq, additional_feats, device, hidden_states=hidden_states, iteration=iteration)
                    LSTM_squence_output = prediction
                    # print('here is test:')
                    # print('input_', input_, input_.shape)
                    # print(LSTM_squence_output)
                else:# for train
                    input_, prediction = self.partially(position_embeddings, tgt_seq, additional_feats, device, hidden_states=hidden_states)
                    # input_ = hidden_states
                    LSTM_squence_output = prediction
                    # print('here is train:')
                    # print('input_', input_, input_.shape)
                    # print(LSTM_squence_output, LSTM_squence_output.shape)
            else:
        ...
        if additional_feats == None and position_embeddings == None:
            ...
            outputs = (LSTM_squence_output, res, embs,)
