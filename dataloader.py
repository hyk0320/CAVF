...
class VideoDataset(Dataset):
    def __init__(self, opt, mode, print_info=False, specific=-1, **kwargs):
        ...
        self.verb = pickle.load(open(opt['verb'], 'rb'))  # dictionary
        self.verb_num = opt["verb_num"]
    ...
    def __getitem__(self, ix):
        ...
        data.update(self.prepare_verb(vid, cap_id, taggings, labels))

        # data.update(self.get_new_module_gt_tokens(data))
    ...
    def prepare_verb(self, vid, cap_id, taggings, labels):
        """
            return tensor:[0,0,0,0,0,1,1,0,...self.verb_num...]
        """
        verb = {}
        verb["verb_label"] = torch.FloatTensor(self.verb[vid][cap_id])
        return verb

        # 获取模块的输出tokens序列
    def get_new_module_gt_tokens(self, data):
        # assert isinstance(data['labels'], Tensor)
        # print(data['labels'].item(), type(data['labels'].item()), type(data['labels']), data['labels'].size(0))
        # labels = labels[1:-1]
        labels_list = data['labels'].numpy().tolist()
        labels_length = len(labels_list)
        new_module_gt = {}
        tmp = []
        tmp.extend(labels_list)
        tmp = tmp + labels_list[1:]
        tmp.append(int(0))
        tmp = tmp + labels_list[2:]
        tmp = tmp + list([0, 0])
        # for index in range(labels_length):
        #     if index < labels_length-2:
        #         tmp.append(labels_list[index])
        #         tmp.append(labels_list[index+1])
        #         tmp.append(labels_list[index+2])
        #     elif index == labels_length-2:
        #         tmp.append(labels_list[index])
        #         tmp.append(labels_list[index+1])
        #         tmp.append(int(0))
        #     else:
        ...
        # print(tmp)
        # print(tmp, type(tmp))
        new_module_gt['new_module_labels'] = torch.LongTensor(tmp)
        return new_module_gt
    ...
    def _prepare_input_ids(self, cap_id, labels, taggings):
        ...
        tokens, labels, taggings, full_sequence = map(
            lambda x: results[x], 
            ["dec_source", "dec_target", "tagging", "full_sequence"]
        )
        ...
        _dict['full_sequence'] = torch.LongTensor(full_sequence)
    ...
    def _make_source_target(self, target, tagging):
        if self.opt['decoding_type'] == 'NARFormer':
            ...
            # results = self._source_target_non_mlm(target[1:-1])
        else:
            # ARFormer
            results = {
    ...
    def _source_target_non_mlm(self, target):
        ...
        dec_target = torch.LongTensor(target)

        if self.mode != 'train':
            # print('before mask dec_source:', dec_source)
            dec_source[dec_source!=Constants.PAD] = Constants.MASK
            # dec_target = dec_target_cp
            # print('dec_source.shape:', dec_source.shape)
            # print('after mask dec_source:', dec_source)

        dec_source = self._padding(dec_source.tolist(), add_eos=False)
        dec_target = self._padding(dec_target.tolist(), add_eos=False)

        # print('after padding dec_source:', dec_source)
        # print('after padding dec_target:', dec_target)
    ...
    def _source_target_mlm(self, target):
        ...
        full_sequence = torch.LongTensor(target)
        ...
        full_sequence = self._padding(full_sequence.tolist(), add_eos=False)
        
        return {'dec_source': dec_source, 'dec_target': dec_target, 'full_sequence': full_sequence}
